  ...
  await page
    .waitForFunction(
      () => {
        ...
        const hasLoader = Boolean(document.querySelector('[class*=Loader_loaderWrapper], [class*=loaderWrapper]'));
        return !hasLoader || text.trim().length > 20;
      },
      undefined,
      { timeout: 30000 },
    )
...
async function waitForText(page, values, timeout = 30000) {
...
async function commonBoardVisual(page, context, fixture, result, resultDir, navLog, token = null, name = 'board') {
  ...
  await waitForText(page, [fixture.project.name, fixture.board.name, fixture.card?.name]);
...
async function testProjectBoard(testCase, env, name) {
  ...
  try {
    if (name === '创建项目') {
      ...
      await waitForText(page, [board.name, 'To Do', 'Doing', 'Done']);
      ...
      await waitForText(page, fixture.project.name);
...
async function testApi(testCase, env, name) {
  ...
  try {
    ...
    const clientId = client.json?.item?.clientId;
    ...
    if (name === 'API认证') {
      ...
      res = await apiRequest('POST', `/api/lists/${fixture.lists.todo.id}/cards`, { position: 65535, name: `api-denied-${runId}` }, null, {
        'X-Client-Id': bad.json?.item?.clientId,
...
async function testAdmin(testCase, env, name) {
  ...
  try {
    ...
    if (['用户列表管理', '修改用户信息', '查看账户活动'].includes(name)) {
      ...
      const manualEmail = `manual_${testCase.caseNo}_${runId}_${Math.random().toString(36).slice(2, 7)}@example.com`.toLowerCase();
      const createUser = await adminApi('POST', '/api/users', { email: manualEmail, password: PASSWORD, name: `Manual ${runId}` });
      let ordinaryCreate = null;
      if (name === '管理员手动添加用户') {
        const ordinary = await registerUser(`manual_ordinary_${testCase.caseNo}`, runId);
        const ordinaryToken = await getAccessToken(ordinary.email, ordinary.password);
        ordinaryCreate = await apiRequest('POST', '/api/users', { email: `ordinary_manual_${runId}@example.com`, password: PASSWORD, name: 'Ordinary Manual' }, ordinaryToken);
      }
      ...
      result.apiEvidence.push(`管理员 POST /api/users 返回 ${createUser.status}，邮箱：${manualEmail}`);
      if (ordinaryCreate) result.apiEvidence.push(`普通用户 POST /api/users 返回 ${ordinaryCreate.status}`);
      const core = await apiRequest('GET', '/api/core-settings-public');
      if (ordinaryCreate && core.json?.item?.demoMode && ordinaryCreate.ok) {
        result.apiEvidence.push(`demoMode：${core.json?.item?.demoMode}，当前 demo 注册用户默认具备管理员能力`);
        result.actual = '管理员可创建项目/用户；但 demo 环境中新注册用户默认具备管理员能力，无法模拟“普通用户不可添加用户”的 Given。';
        result.status = 'blocked';
      } else {
        const ok = project.id && createUser.ok && (!ordinaryCreate || ordinaryCreate.status === 403 || ordinaryCreate.status === 401 || ordinaryCreate.status === 404);
        result.actual = ok ? '管理员可创建项目/用户，普通用户手动添加用户被拒绝。' : '管理员创建项目/用户或普通用户拒绝路径未达到预期。';
...
async function testSettings(testCase, env, name) {
  ...
  try {
    ...
    if (name === '个人资料设置') {
      ...
      const res = await apiRequest('PATCH', `/api/users/${userId}/username`, { username: `qyj${runId.replace(/[^a-z0-9]/gi, '').slice(-10)}`, currentPassword: ADMIN.password }, token);
