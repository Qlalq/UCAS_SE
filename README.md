...
- 任务二：本地运行 `python generate_scripts.py`，驱动 `https://demo.4gaboards.com/` 为全部72个测试场景生成测试脚本。
- 演示录屏：见 `video/project1.mp4`（任务一）与 `video/project2.mp4`（任务二）。

### 1.2 账号信息

任务二在大模型 prompt 内预置的统一测试账号，供大模型参考：

| 用途              | 用户名             | 密码           |
| ----------------- | ------------------ | -------------- |
| 任务二 Agent 登录 | `3652750340@qq.com` | `abc.147258369` |

此外，执行过程中也会由 Agent 通过 `/api/access-tokens` 接口动态注册的临时账号（用于跨用户权限验证、Given 前置条件构造），注册邮箱形如 `user_module_*_<runId>@example.com`，执行结束后留存在演示实例用户表中。
...
| 成员   | 负责内容                                                             | 贡献占比 |
| ------ | -------------------------------------------------------------------- | -------: |
| 钱永杰 | 任务一 RAG 检索模块、混合检索权重调优、72 测试点校对                 |      25% |
| 童泽露 | 任务一 Prompt 设计与四种模式对比实验、tests.json 校对                |      20% |
| 木热   | 任务二 Tool Use Agent 架构、code_execution 工具实现、playwright 调度 |      25% |
| 刘家恒 | 任务二 file_search 工具实现、12 个分类的批量执行与回归               |      20% |
