...
本报告基于的项目在考试结束后已接受过现场检查，确认系统按文档描述运行，任务一、任务二的产物与录像可现场复现。后续如再补充提交检查材料，以本仓库与录像为准。
...
本课程作业的完整工程根目录为 `lecture/SE_final/`，本地路径 `D:\workspace\claude\lecture\SE_final`。

同步托管在 GitHub，仓库地址：`https://github.com/Qlalq/UCAS_SE`。仓库内含完整 Git 历史与 `结题报告_第4组.docx` 等结题材料，可在线查看 commit 时间线与文件结构。
...
| 成员 | Scrum 角色 | 任务主线 |
| --- | --- | --- |
| 钱永杰 | SM（Scrum Master / 组长） | 整体节奏与思路规划、Sprint 计划与复盘、开题/中期/结题 PPT、结题报告与文档统筹、仓库初始化 |
| 木热 | 开发团队 | 任务一 RAG 兜底与混合检索工程化、任务二 Tool Use Agent 全栈开发、批量执行与回归 |
| 童泽露 | PO（产品负责人 · 任务一） | 任务一需求边界、12 分类与配额、prompt 模板、tests.json 校对 |
| 刘家恒 | PO（产品负责人 · 任务二） | 任务二智能体能力边界、工具契约、用例执行结果验收 |

四名成员工作内容按 Scrum 角色对等分担：SM 承担文档统筹与节奏管理的最高密度，PO-任务一与 PO-任务二分别在 Sprint 1-2 与 Sprint 3-4 期间切入产品主线，开发在两条任务线间全程支撑实现与回归。每位成员的产出与代码库模块归属一一对应，可直接在仓库内追溯。
...
Git 协作贯穿整个项目期，仓库记录了从仓库初始化到结题打磨的全部演进过程。每天每位成员根据 Sprint Backlog 推进自己负责的卡，提交时按 `feat(task1|task2)` / `tune` / `fix` / `test` / `docs` / `chore(scrum)` 这几类前缀区分意图，方便后续按类别筛选。SM 同步把仓库节奏同步到站会与 Sprint 复盘，形成「代码演进 + 文档沉淀 + 节奏管理」的闭环。
